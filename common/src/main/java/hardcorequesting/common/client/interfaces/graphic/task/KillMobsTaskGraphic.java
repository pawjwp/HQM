...
public class KillMobsTaskGraphic extends IconTaskGraphic<KillMobsTask.Part> {
    ...
    protected void drawElementText(GuiGraphics graphics, KillMobsTask.Part part, int index, int x, int y) {
        ...
        if (killed == part.getCount()) {
            gui.drawString(graphics, Translator.translatable("hqm.mobTask.allKilled").withStyle(ChatFormatting.DARK_GREEN), x, y, 0.7F, HQMConfig.TEXT_NORMAL);
        } else {
            gui.drawString(graphics, Translator.translatable("hqm.mobTask.partKills", killed, (100 * killed / part.getCount())), x, y, 0.7F, HQMConfig.TEXT_NORMAL);
        }
        gui.drawString(graphics, Translator.translatable("hqm.mobTask.totalKills", part.getCount()), x, y + 6, 0.7F, HQMConfig.TEXT_NORMAL);
    }
